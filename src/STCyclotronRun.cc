...
STCyclotronRun::STCyclotronRun()
  : G4Run(),fTotalEnergyDepositTarget(0.),fTotalEnergyDepositFoil(0.), fParticleTarget(0),fTargetThickness(0.),fTargetDiameter(0.),fTargetVolume(0.), fFoilVolume(0.),fFoilThickness(0.), fPrimariesPerEvent(0),fTimePerEvent(0),fBeamName(""),fBeamCurrent(0.),fBeamEnergy(0.)
{ }
  ...
  for (it = fPrimaryIsotopeCountTarget.begin(); it != fPrimaryIsotopeCountTarget.end(); it++)
    {
      ...
      //* (1. - std::exp(-irradiationTime*3600*decayConstant));
      ...
      fPrimaryActivityTarget[name]= fPrimaryIsotopeEOBTarget[name]*conv*decayConstant;
      ...
      if(store)
	{
	  ...
	  fOutPut1 << primaryActivityTotal << "total activity" << G4endl;
	  fOutPut1 << "------------------------" << G4endl; 
	}        
