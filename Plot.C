...
void Plot(){
  ...
  TCanvas *RangeTarget = new TCanvas("Range ", "Range in the target");

  TH1D *RangeProfileTarget = (TH1D*)f->Get("H19;1");
  RangeProfileTarget->GetXaxis()->SetTitle("Range (mm)");
  RangeProfileTarget->GetYaxis()->SetTitle("number of protons");
  RangeProfileTarget->SetTitle("Primary particles range in the target");
  
  RangeProfileTarget->GetXaxis()->SetMaxDigits(2);
  RangeProfileTarget->GetYaxis()->SetMaxDigits(3);

  //Normalize histogram per bin width and per incoming particle.
  norm_th1_per_bin_width_per_primaries(RangeProfileTarget,total_primaries);

  RangeProfileTarget->Draw("H");
  RangeTarget->Print("./Results/BeamData/Range.pdf");
